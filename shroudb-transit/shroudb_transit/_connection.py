...
Do not import directly — use `shroudb_transit.ShroudbTransitClient` instead.
...
class _Connection:
    ...
    async def _read_frame(self) -> Any:
        ...
        if tag == "+":
            ...
            raise ShroudbTransitError._from_server(code, message)
        elif tag == ":":
            return int(payload)
        elif tag == "$":
            length = int(payload)
            ...
            raise ShroudbTransitError(f"INTERNAL", f"Unknown response type: {tag}")
