...
| `ping()` | Ping-pong |
| `put(index, id, plaintext_b64, **kwargs)` | Tokenize plaintext and store the blind tokens under the given entry ID |
| `search(index, query, **kwargs)` | Search a blind index. Tokenizes the query, generates blind tokens, and compares against stored entries. |
