...
Do not import directly — use `shroudb.ShroudbClient` instead.
...
class _Connection:
    ...
    async def _read_frame(self) -> Any:
        ...
        if tag == "+":
            ...
            raise ShroudbError._from_server(code, message)
        elif tag == ":":
            return int(payload)
        elif tag == "$":
            length = int(payload)
            ...
            raise ShroudbError(f"INTERNAL", f"Unknown response type: {tag}")
