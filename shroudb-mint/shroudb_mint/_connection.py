...
Do not import directly — use `shroudb_mint.ShroudbMintClient` instead.
...
class _Connection:
    ...
    async def _read_frame(self) -> Any:
        ...
        if tag == "+":
            ...
            raise ShroudbMintError._from_server(code, message)
        elif tag == ":":
            return int(payload)
        elif tag == "$":
            length = int(payload)
            ...
            raise ShroudbMintError(f"INTERNAL", f"Unknown response type: {tag}")
