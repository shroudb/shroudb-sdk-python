...
Do not import directly — use `shroudb_pulse.ShroudbPulseClient` instead.
...
class _Connection:
    ...
    async def _read_frame(self) -> Any:
        ...
        if tag == "+":
            ...
            raise ShroudbPulseError._from_server(code, message)
        elif tag == ":":
            return int(payload)
        elif tag == "$":
            length = int(payload)
            ...
            raise ShroudbPulseError(f"INTERNAL", f"Unknown response type: {tag}")
