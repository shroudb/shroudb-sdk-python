...
Do not import directly — use `shroudb_sentry.ShroudbSentryClient` instead.
...
class _Connection:
    ...
    async def _read_frame(self) -> Any:
        ...
        if tag == "+":
            ...
            raise ShroudbSentryError._from_server(code, message)
        elif tag == ":":
            return int(payload)
        elif tag == "$":
            length = int(payload)
            ...
            raise ShroudbSentryError(f"INTERNAL", f"Unknown response type: {tag}")
